// server.js - Complete Backend for Creobotics
...
app.post('/api/auth/register', async (req, res) => {
    const { name, email, school, password, role } = req.body;
    ...
    try {
        ...
        const [result] = await db.query(
            `INSERT INTO users (name, email, school, password_hash, role, profile_complete, is_admin, is_teacher) 
             VALUES (?, ?, ?, ?, ?, 0, ?, ?)`,
            [name, email.toLowerCase(), school, hashedPassword, userRole, userRole === 'admin' ? 1 : 0, userRole === 'teacher' ? 1 : 0]
        );
    ...
    try {
        ...
        const [users] = await db.query(
            `SELECT id, email, name, school, password_hash, avatar_id, nickname, 
                    profile_complete, is_admin, is_teacher, role, class_code, teacher_email 
        ...
        if (role && user.role !== role) {
            ...
            if (role === 'student' && (user.is_admin || user.is_teacher)) {
                // Admins and teachers can also login as student
                // But if they specifically selected student, allow it
            }
        }
    ...
    try {
        const [users] = await db.query(
            `SELECT id, email, name, school, avatar_id, nickname, profile_complete, 
                    is_admin, is_teacher, role, class_code, teacher_email 
             FROM users WHERE id = ?`,
            [req.userId]
        );
...
app.get('/api/admin/serial-keys', authenticate, async (req, res) => {
    ...
    try {
        const [keys] = await db.query(
            'SELECT id, code, grade, used_by, used_at, created_at FROM serial_keys ORDER BY created_at DESC'
        );
        res.json(keys);
    } catch (error) {
...
app.post('/api/admin/serial-keys', authenticate, async (req, res) => {
    ...
    try {
        for (let i = 0; i < count; i++) {
            ...
            await db.query(
                'INSERT INTO serial_keys (code, grade, created_by) VALUES (?, ?, ?)',
                [code, grade, req.userId]
            );
...
app.post('/api/serial/redeem', authenticate, async (req, res) => {
    ...
    try {
        const [keys] = await db.query(
            'SELECT id, grade, used_by FROM serial_keys WHERE code = ?',
            [normalizedCode]
        );
        ...
        try {
            ...
            await connection.query(
                `INSERT INTO access (user_id, serial_key, grade, activated_at, expires_at) 
                 VALUES (?, ?, ?, NOW(), ?)
                 ON DUPLICATE KEY UPDATE serial_key = ?, grade = ?, activated_at = NOW(), expires_at = ?`,
                [req.userId, normalizedCode, key.grade || 4, expiresAt, normalizedCode, key.grade || 4, expiresAt]
            );
...
app.get('/api/access/status', authenticate, async (req, res) => {
    try {
        const [access] = await db.query(
            'SELECT grade, serial_key, activated_at, expires_at FROM access WHERE user_id = ?',
            [req.userId]
        );
...
app.post('/api/teacher/redeem', authenticate, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await connection.query(
                'UPDATE users SET is_teacher = 1, role = "teacher" WHERE id = ?',
                [req.userId]
            );
        ...
        if (currentUser.is_admin) {
            query = `
                SELECT id, name, email, school, nickname, avatar_id, is_admin, is_teacher, 
                       class_code, teacher_email, profile_complete, role
                FROM users 
